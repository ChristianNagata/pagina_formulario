...
def campo_tem_algum_numero(valor_campo, nome_campo, lista_de_erros):
    ...
    if any(char.isdigit() for char in valor_campo):
        lista_de_erros[nome_campo] = 'Campo não aceita números'


def data_ida_maior_que_data_volta(data_ida, data_volta, lista_de_erros):
    """Verifica se data de ida é maior do que a data de volta."""
...
def data_ida_menor_que_data_hoje(data_ida, data_pesquisa, lista_de_erros):
    """Verifica se a data de ida é manor do que a data de hoje."""
    if data_ida < data_pesquisa:
        lista_de_erros['data_ida'] = 'Data de ida não pode ser menor do que a data de hoje.'
